...
export function CodePreview({ files }: CodePreviewProps) {
  ...
  const filesByFolder: Record<string, FileWithContent[]> = {};
  files.forEach((file) => {
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-xs ${
                lineWrap ? 'bg-gray-500/20 text-gray-400' : 'bg-red-500/20 text-red-400'
              } border border-gray-500/30 hover:bg-gray-500/30 transition-all`}
            >
